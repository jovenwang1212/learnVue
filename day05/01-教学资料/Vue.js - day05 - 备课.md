...
> 1. 接下来我们要学习一个重要的知识点，Vue路由。Vue路由是什么呢？这个得从锚链接说起。
> 2. 我们经常用超链接a href=url用来跳转到新的页面对吧, 而锚链接是超链接的一种，只是a href值并不一个url，是一个`#字符串`，能快速滚动页面某个位置。可能有同学忘记了，来我们复习一下。
> 3. 举例锚链接，能快速滚到顶部。注意到url上加了`#top`，这个是什么，就是我们的锚链接的值吧，在url上这个‘#字符串’叫做hash.那么我们也可以直接修改hash来滚动到页面某个位置对吧。
> 4. 来，我们在页面底部增加一个id=buttom的div块，修改url的hash，能快速滚动到bottom
> 5. 而且js本身就支持，用onhashchange方法监听到hash改变。location.hash获取当前页面的hash
...
> 2. MPA就是我们传统的页面组织结构，比如百度搜索，搜索结果点开都会打开新的页面，URL改变，且加载新的页面需要loading
> 3. 而SPA常见于后台管理系统 https://taylorchen709.github.io/vue-admin>，这个是后端管理系统，当我们切换菜单的时候，注意整个页面有刷新吗？还有url有变化吗？只有hash变了。页面加载速度怎么样？很快对吧。我们点菜单就展示不同的内容。检查HTML内容，点击菜单改变的只是section.content-container的内容吧。
> 4. 所以SPA是一个外壳页面和多个页面片段构成的. 大部分的功能都在一个页面上，局部切换页面。
> 5. 对于SPA我们得在页面初始化时初始化外壳页和一个至少一个页面片段，所以首屏加载慢，但是页面切换，只用更新另一个页面片段，所以切换快。这种特性，对于后台管理比较适合。比如HR使用OA系统，可以一天都在使用OA，可能首次打开慢一点，但是后续切换页面快，比较适合。
...
> 1. 我们注意到，刚才的那个管理端，切换页面时hash在变。我们来模仿实现一个改变hash，切换页面的效果
> 2. 节省时间，我已经写好了页面，菜单和内容区，点菜单只定位到具体的div. 这里我们需要只展示其中一个div.需要结合onhashchange
...
> 1. 我们刚才用hash实现了一个丑陋的SPA，onhashchange这块实现改变hash切换页面，这个就是Vue路由的基本原理，*只是Vue路由它更强大好用*。
> 2. 那么Vue路由是用来构建单页应用
> 3. vue路由是一个vue插件，vue-router.js，并不包含在vue.js里面。渐进式，有些项目不用啊。
...
> 1.  我们做悦听播放器时，一直模仿的网易云音乐网站，就是SPA的。发现音乐，我的音乐，朋友和下载四个菜单切换时，只是改变了hash. ok, 我们花一节课时间来高仿一个网易云音乐。
> 2. copy路由基本使用的代码,router-link和路由规则一一对应，组件内用图片代码即可完成。
> 3. 需要理解到Vue路由的精髓其实就是改变hash，显示对应的组件。只不过我们这里组件内容用图片占位的。
...
> 1. 接下来我们来完成一个Vue路由的案例，黑云音乐。
> 2. 演示功能，搜歌，播歌，mv播放及评论。
> 3. 注意到下方部分内容分别是搜歌，播歌，mv播放及评论，且随hash变化的。这个就是路由对吧。
> 4. 查看模板，搜索结果，播放，mv播放及评论分别是四个页面，有什么想法，是不是应该把这个四个页面，整成四个页面片段，也就是封装成组件？
> 5. 我们先来结合学习过的路由知识，完成改变hash, 显示不同组件的功能
> 6. 然后是不是应该把四个页面作为组件引入到index.html. 检查四个页面的结构，script template引入，样式引入。
> 7. 所有代码都放在index.html中
...
> 4. 要实现声明式导航同样的效果，我们就router.push()，copy声明式导航的值
...
> 4. 组件也是Vue实例，Vue生命周期图标也适合组件，但并不是所有生命周期钩子this都可以访问到data属性。我们看一下，初始化四个钩子哪一个能尽早获取
...
>2. 场景引入，如果我需要让农历出生日期和公历出日期按同样格式‘20190605’输出？怎么做？
>3. 两个计算属性，计算逻辑是一致的？有没有更好的解决方案呢？我们能不 能把共同的计算逻辑提取出来呢
>4. 参考文档，定义过滤器，解决日期的显示
>5. 如果这里有多10个日期按同样的格式显示，用过滤器很简单。如果是计算属性得多写10个计算属性吧。
>6. 我们说如果属性不想原样输出就用计算属性，这里改一下，除了文本格式化外，其他非原样输出用计算属性。
>7. 过滤器只能作在{{}}和v-bind里面
>8. 解释过滤器的使用{{数据|过滤器}} 过滤器方法接受一个参数，就是它所作用的数据，在方法里面可以拿到这个数据，处理后，return，return的数据就是最终显示的数据
...
> 1. 首先我们来看一个概念，单文件组件，什么是单文件组件呢，拆开来看，组件我们认识吧？我们说组件是独立的功能模块，包含html,css,js。单文件组件就是一个文件包含组件的所有内容，所以组件包含html，css和js
